...
export const simulateBinanceTransfer = async (req, res) => {
  ...
  try {
    ...
    const timestamp = Date.now();
    const queryString = `timestamp=${timestamp}`; // Firma solo con el timestamp para evitar duplicación de parámetros
    ...
    bodyParams.append('asset', cryptoSymbol);
    bodyParams.append('amount', amount);
    bodyParams.append('type', `${fromAccountType}_${toAccountType}`);
    ...
    const response = await fetch(url, {
      ...
      body: bodyParams.toString(),  // Enviar los parámetros en el cuerpo
    });
