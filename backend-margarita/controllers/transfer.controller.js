...
class BitcoinWallet {
    ...
    addFunds(amount) {
      this.balance += amount;
      console.log(`Fondos agregados: ${amount} satoshis. Saldo actual: ${this.balance} satoshis.`);
    }
  ...
  export const simulateTransfer = async (req, res) => {
    ...
    try {
      ...
      const [userCrypto] = await connection.query('SELECT balance FROM cryptos WHERE id_usuario = ?', [id_usuario]);
  
      if (!userCrypto || userCrypto[0].balance < amountToTransfer) {
        return res.status(400).json({ error: 'Saldo insuficiente.' });
      }
  
      // Obtener el id_usuario correspondiente a toAddress
      const [recipient] = await connection.query('SELECT id_usuario FROM cryptos WHERE address = ?', [toAddress]);
  
      if (!recipient.length) {
        return res.status(400).json({ error: 'La dirección de destino no es válida.' });
      }
      ...
      await connection.query('UPDATE cryptos SET balance = balance - ? WHERE id_usuario = ?', [amountToTransfer, id_usuario]);
  
      // Sumar el monto transferido al balance del usuario que recibe
      await connection.query('UPDATE cryptos SET balance = balance + ? WHERE id_usuario = ?', [amountToTransfer, recipient[0].id_usuario]);
  
      // Registrar la transacción en la tabla de transacciones
      await connection.query('INSERT INTO transactions (id_usuario, amount, type) VALUES (?, ?, "transfer")', [id_usuario, amountToTransfer]);
  ...
  export const addFunds = async (req, res) => {
    ...
    try {
      ...
      await connection.query('INSERT INTO transactions (id_usuario, amount, type) VALUES (?, ?, "add")', [id_usuario, amount]);
  
      // Actualizar el balance del usuario en la tabla de cryptos
      await connection.query('UPDATE cryptos SET balance = balance + ? WHERE id_usuario = ?', [amount, id_usuario]);
  ...
  export const createAddress = async (req, res) => {
    ...
    // Verificar si se recibió id_usuario
    if (!id_usuario) {
    ...
    try {
        ...
        const newAddress = '1A' + Math.random().toString(36).substring(2, 15);
...
export const getTransactions = async (req, res) => {
  ...
  try {
    // selecciona las transacciones del usuario
      const [transactions] = await connection.query('SELECT * FROM transactions WHERE id_usuario = ?', [id_usuario]);
